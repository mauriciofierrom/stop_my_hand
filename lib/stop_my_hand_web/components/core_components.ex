defmodule StopMyHandWeb.CoreComponents do
  @moduledoc """
  ...
  def input(%{type: "checkbox", value: value} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-4 text-sm leading-6 text-light">
        ...
        <input
          ...
          value="true"
          checked={@checked}
          class="rounded border-stroke text-primary focus:ring-0"
          {@rest}
        />
        <%= @label %>
      </label>
  ...
  def dropdown_item(assigns) do
    ~H"""
    <div id={@id} class="block px-4 py-2 text-sm text-gray-700 cursor-pointer" role="menu-item" tab-index="-1">
      <%= render_slot(@inner_block) %>
    </div>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(StopMyHandWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(StopMyHandWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  attr :status, :atom, values: [:online, :offline], doc: "the status to indicate"
  def status_indicator(assigns) do
    status = assigns.status
    base = ["w-2.5 h-2.5 rounded-full"]
    assigns = assign(assigns, :le_class, [status_indicator_class(status) | base])
    ~H"""
